...
%saber si la matriz dada esta mal condicionada o no, lo que produciría una
...
%calcular la solucion de los sistemas lineales, y los vectores x tendrán 
%solución o no, según lo que obtuvimos anteriormente, es decir, si los 
%numeros de condicion de nuestras matrices son muy grandes y/o si los 
%determinantes son 0, entonces no será posible hallar una solución para 
%los sistemas planteados, de forma que se podría obtener como resultado:
%NaN y/o Inf.

%4. Vector Solución que corresponde a cada sistema lineal.
%Para x=A\b
...
%Al comparar ambos métodos podemos ver que el más eficiente para la máquina
%es A\B, dado que éste no busca la inversa de la matriz para luego
...
%que el de B2 se aproxima a 0, más no es 0, y es por eso que podemos
%calcular una solución para el sistema, y tomando en cuenta que nuestra b
%tiene un error de 10^-7, entonces la matriz B2 es la más apropiada para
%correr el modelo matematico.
%Dado que B1 esta mal condicionada y su determinante es 0, el sistema no
%puede tener solución y matlab arroja como solucion :para el primer método 
% NaN, e Inf para el segundo metodo de solucion del sistema.
%mientras que para B2, se obtiene una solucion.


%Finalmente podemos ver, que la relación que tiene el determinante, la
%condicion y la norma de una matriz esta en que:
%Si el numero de condición de una matriz es muy grande, entonces su
%determinante será pequeno, y con la norma infinito, tenemos la relación
%que tiene esta con el numero de condición, dado que a través de esta se
%calcula el numero de condición, y si ésta es indeterminada pues no se podrá
%calcular el numero de condición, como por ejemplo norm(inv(A),inf)=Inf,
